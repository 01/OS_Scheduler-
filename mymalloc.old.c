  ...
  int * ptr = manager->Reserved_Page_Table; // Point to begining of Page Table
      ...
      if(currentThread->heap == NULL){
        if(numThreads >= MAX_THREADS){
          return NULL; //too many threads, no more
        }
        int freePageIndex = findFreeHeapPage();
        if(freePageIndex < 0){
          // need to swap
        }
        currentThread->threadID = ++numThreads;
      ...
      if(bitShift(*(int *) track, -16) != currentThread->threadID){
      ...
      while(track < (track + PAGE_SIZE)){
        ...
        if (*(int *)track > 0 && currentBlocksize >= size){  //check if location is free
          ...
          *(int *)track = bitShift(size, 8) + currentThread->threadID;  //negative to signify allocated space
          int * tmp = track + size + 2;
          *tmp = bitShift((currentBlocksize - size -2), 8) + currentThread->threadID;
